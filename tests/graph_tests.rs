use aequa::graph::{Graph, GraphError};
use aequa::XffValue;
...
fn test_graph_add_remove_nodes() {
    ...
    assert_eq!(n3, 1);
    assert_eq!(graph.get_node(n3).unwrap().payload.as_string().unwrap(), "node3");
}
...
fn test_graph_root_protection() {
    ...
    assert_eq!(graph.get_node(0).unwrap().payload.as_string().unwrap(), "root");
}
...
fn test_graph_export_topo() {
    ...
    for (name, parents) in export {
        match name.as_str() {
            "a" => assert!(parents.is_empty()),
            "b" => assert_eq!(parents, vec!["a"]),
            "c" => assert_eq!(parents, vec!["b"]),
            _ => panic!("Unexpected node name"),
        }
    }
}
